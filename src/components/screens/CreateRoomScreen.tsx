...
import { DEFAULT_SETTINGS, ROLE_META, type RoomSettings, type Role } from '../../types';
...
export function CreateRoomScreen({ onBack }: CreateRoomScreenProps) {
  const { myName, createRoom, error, clearError } = useGameStore();
  ...
  const handleCreate = async () => {
    if (loading) return;
    setLoading(true);
    await createRoom(myName, settings);
    setLoading(false);
  };

  return (
    <div className="min-h-full h-full flex flex-col px-5 py-6 safe-top safe-bottom">
      {/* Header */}
      <div className="flex items-center gap-3 mb-5 shrink-0">
        ...
        <h1 className="font-display text-xl font-bold">Новая комната</h1>
      </div>

      {error && (
        <div className="mb-3 p-3 rounded-xl bg-red-500/10 border border-red-500/25 text-red-400 text-sm text-center shrink-0" onClick={clearError}>
          {error}
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col gap-4 overflow-y-auto pb-4 pr-1">
        {/* Roles */}
        <h2 className="text-xs font-bold tracking-wider uppercase text-gray-500">Роли</h2>
        ...
        <div className="p-2.5 rounded-xl bg-civilian/10 border border-civilian/20 text-sm text-civilian text-center">
          Минимум {minPlayers} игроков. Остальные — мирные жители.
        </div>

        {/* Voting */}
        <h2 className="text-xs font-bold tracking-wider uppercase text-gray-500">Голосование</h2>
        <div className="flex items-center justify-between p-3 rounded-xl bg-bg-card border border-white/8">
          <div className="flex flex-col">
            <span className="text-sm font-semibold">
              {settings.votingType === 'open' ? 'Открытое' : 'Закрытое'} голосование
            </span>
            <span className="text-[11px] text-gray-500">
              {settings.votingType === 'open' ? 'Все видят голоса в реальном времени' : 'Результат после завершения'}
            </span>
          </div>
          <button
        ...
        {/* Timers */}
        <h2 className="text-xs font-bold tracking-wider uppercase text-gray-500">Таймеры</h2>
        <div className="flex flex-col gap-2 p-3 rounded-xl bg-bg-card border border-white/8">
          <div className="flex items-center justify-between mb-1">
            <div className="flex flex-col">
              <span className="text-sm font-semibold">Включить таймеры</span>
              <span className="text-[11px] text-gray-500">Информационные — фазу завершают игроки</span>
            </div>
            <button
        ...
        {/* Additional */}
        <h2 className="text-xs font-bold tracking-wider uppercase text-gray-500">Дополнительно</h2>
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between p-3 rounded-xl bg-bg-card border border-white/8">
            <div className="flex flex-col">
              <span className="text-sm font-semibold">Раскрывать роль убитого</span>
              <span className="text-[11px] text-gray-500">Показывать роль при смерти</span>
            </div>
            <button
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-bg-card border border-white/8">
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-gray-500">Голосовые подсказки на устройстве хоста</span>
            </div>
            <button
      ...
      <button
        ...
        disabled={loading}
        className="h-[52px] rounded-[14px] bg-mafia text-white font-semibold text-base disabled:opacity-40 hover:brightness-110 active:scale-[0.98] transition-all mt-4 shrink-0"
      >
